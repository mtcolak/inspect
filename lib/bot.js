...
class Bot extends EventEmitter {
    ...
    _retryConnectLoop() {
        ...
        const maxDelay = this.settings._max_reconnect_delay || 5 * 60 * 1000;
        const baseDelay = this.settings._initial_reconnect_delay || 60000;
        const delay = Math.min(baseDelay * this.retryAttempt, maxDelay);

        winston.info(`[${this.username}] 🔄 Reconnect attempt #${this.retryAttempt} in ${delay / 1000}s...`);

        setTimeout(() => {
            if (!this.isConnected) {
                winston.info(`[${this.username}] 🔁 Trying to reconnect (Attempt #${this.retryAttempt})`);
    ...
    _startHeartbeatCheck() {
        const interval = this.settings._heartbeat_interval || 5 * 60 * 1000;

        setInterval(() => {
            if (!this.isConnected || !this.steamClient.steamID) {
                winston.warn(`[${this.username}] 💓 Heartbeat check failed. Reconnecting...`);
                this._retryConnectLoop();
            }
        }, interval);
    }
